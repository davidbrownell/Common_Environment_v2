...
class Freezer(object):
    ...
    def _WriteModules(self, fileName, finder):
        ...
        # ----------------------------------------------------------------------
        def PruneIgnoredChildren(root, children):
            index = 0
            while index < len(children):
                fullpath = os.path.join(root, children[index])

                if os.path.isdir(fullpath) and children[index] in [ "Generated", 
                                                                  ]:
                    del children[index]
                else:
                    index += 1

            return ignorePatterns(root, children)
